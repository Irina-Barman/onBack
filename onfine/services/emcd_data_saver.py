...
class EMCDDataSaver:
    ...
    def _validate_income_data(self, data: Dict[str, Any]) -> None:
        ...
        if "income" not in data or not isinstance(data["income"], list):
            raise ValueError("Invalid income data: missing or invalid 'income' list")
        for item in data["income"]:
            required_fields = ["code", "timestamp", "gmt_time", "income", "type", "total_hashrate"]
            for field in required_fields:
                if field not in item:
                    raise ValueError(f"Missing field '{field}' in income item")
            if not isinstance(item["income"], (int, float)):
                raise ValueError(f"Invalid 'income' type: {type(item['income'])}")
            if not isinstance(item["timestamp"], int):
                raise ValueError(f"Invalid 'timestamp' type: {type(item['timestamp'])}")
    ...
    def _validate_payouts_data(self, data: Dict[str, Any]) -> None:
        ...
        if "payouts" not in data or not isinstance(data["payouts"], list):
            raise ValueError("Invalid payouts data: missing or invalid 'payouts' list")
        for item in data["payouts"]:
            required_fields = ["timestamp", "gmt_time", "amount", "txid"]
            for field in required_fields:
                if field not in item:
                    raise ValueError(f"Missing field '{field}' in payouts item")
            if not isinstance(item["amount"], (int, float)):
                raise ValueError(f"Invalid 'amount' type: {type(item['amount'])}")
            if not isinstance(item["timestamp"], int):
                raise ValueError(f"Invalid 'timestamp' type: {type(item['timestamp'])}")
    ...
    def save_all(self) -> None:
        ...
        try:
            ...
            for coin_id in account_data.get("coins", {}):
