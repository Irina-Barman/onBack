...
Основные функции:
- Инвестирование средств пользователя в выбранное майнинговое оборудование с
    делением суммы на реферальную часть (50%) и чистую инвестицию (50%).
...
- invest(user: Any, equipment_id: int, gross_amount: Decimal) -> EquipmentInvestment
    Выполняет инвестицию пользователя в оборудование, списывает средства,
        начисляет реферальную часть и чистую инвестицию.
    Возвращает объект EquipmentInvestment. Применяет декоратор @ledger для
        логирования в системе учёта.
