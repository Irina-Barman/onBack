"""
Модуль сервиса покупок.

Основные функции:
- Создание записи о покупке с проверкой наличия пакета.
- Создание покупки без комиссии сети с подготовкой и публикацией транзакции USDT (ERC20, BEP20, TRC20).
- Управление блокировками кошельков на время выполнения покупки.
- Оценка и резервирование газа для транзакций.
- Отправка заданий на пополнение газа через Kafka.
- Логирование операций и обработка ошибок с откатом транзакций при сбоях.

Используемые константы:
- USDT_CONTRACTS (dict): Адреса контрактов USDT для сетей ERC20, BEP20, TRC20.
- PLATFORM_RECIPIENTS (dict): Адреса платформы для получения USDT в соответствующих сетях.

Зависимости:
- logging: Для логирования операций и ошибок.
- os: Для доступа к переменным окружения.
- datetime.datetime: Для отметок времени создания записей.
- decimal.Decimal: Для точных денежных вычислений.
- typing.Optional: Для указания необязательных параметров.
- onfine.blockchain.providers.ProviderManager: Для работы с блокчейн-провайдерами.
- onfine.extensions.db: Сессия SQLAlchemy для работы с базой данных.
- onfine.models.package.Package: Модель пакета.
- onfine.models.platform_gas_expense.PlatformGasExpense, GasExpenseStatus: Модель и статус расходов газа платформы.
- onfine.models.purchase.Purchase, PurchaseStatus, PurchaseStep: Модель покупки и её статусы.
- onfine.models.wallet.Wallet: Модель кошелька пользователя.
- onfine.services.locks: Для управления блокировками кошельков.
- onfine.services.locks.WalletLockPurpose: Цели блокировок кошельков.
- ..utils.kafka_producer: Для отправки сообщений в Kafka.

Класс PurchaseService:

- create_purchase(user_id: int, item_id: int, payment_wallet: Optional[str] = None) -> Purchase
    Создаёт запись о покупке пакета для пользователя.
    Проверяет существование пакета, фиксирует сумму и газ (заглушка).
    Логирует успешное создание или ошибки, при ошибках откатывает транзакцию.
    Выбрасывает исключения ValueError или Exception при ошибках.

- create_purchase_gasless(user, package, network: str) -> Purchase
    Создаёт покупку без комиссии сети с подготовкой транзакции USDT.
    Определяет адреса контрактов и платформы по сети.
    Находит кошелёк пользователя для указанной сети.
    Получает провайдера сети и оценивает необходимые средства газа с запасом.
    Блокирует кошелёк на время выполнения операции.
    Формирует и подписывает транзакцию (различается для EVM-сетей и TRON).
    Создаёт запись покупки и связанный расход газа платформы.
    Отправляет задачу на пополнение газа через Kafka.
    Возвращает объект Purchase.

Исключения:
- ValueError: При отсутствии пакета или кошелька в указанной сети.
- Exception: При ошибках создания покупки или взаимодействия с базой данных.
